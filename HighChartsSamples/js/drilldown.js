(function (g) {
	...
	l.prototype.addSingleSeriesAsDrilldown = function (a,
		b) {
		var d = a.series,
		c = d.xAxis,
		f = d.yAxis,
		...
		i(d.chart.series, function (a) {
			if (a.xAxis === c)
				w.push(a), g.push(a.userOptions), a.levelNumber = a.levelNumber || 0
		});
		h = {
			levelNumber : k,
		...
		h.levelNumber = k + 1;
		if (c)
			c.oldPos = c.pos, c.userMin = c.userMax = null, f.userMin = f.userMax = null;
		if (d.type === h.type)
			h.animate = h.animateDrilldown || t, h.options.animation = !0
	};
	...
	l.prototype.showDrillUpButton = function () {
		...
		this.drillUpButton ? this.drillUpButton.attr({
			text : b
		}).align() : (f = (c = d.theme) && c.states, this.drillUpButton = this.renderer.button(b, null, null, function () {
					a.drillUp()
				}, c, f && f.hover,
					f && f.select).attr({
	...
	l.prototype.drillUp = function () {
		for (var a = this, b = a.drilldownLevels, d = b[b.length - 1].levelNumber, c = b.length, f = a.series, h = f.length, e, g, j, k, l = function (b) {
			...
			c = c || a.addSeries(b, !1);
			if (c.type === g.type && c.animateDrillupTo)
			...
			if (e = b[c], e.levelNumber === d) {
				...
				if (j.type === g.type)
					j.drilldownLevel = e, j.options.animation = a.options.drilldown.animation, g.animateDrillupFrom && g.animateDrillupFrom(e);
				j.levelNumber = d;
				g.remove(!1);
				if (j.xAxis)
					k = e.oldExtremes, j.xAxis.setExtremes(k.xMin, k.xMax, !1), j.yAxis.setExtremes(k.yMin, k.yMax, !1)
			}
		this.redraw();
	...
	function (a) {
		...
		if (!a)
			i(d, function (a) {
				if (b.userOptions === a.lowerSeriesOptions)
					c = a.shapeArgs
	...
	g.Point.prototype.doDrilldown = function (a) {
		for (var b = this.series.chart, d = b.options.drilldown, c = (d.series || []).length, f; c-- && !f; )
	...
	q(g.Point.prototype, "init", function (a, b, d, c) {
		...
		if (f.drilldown) {
			if (g.addEvent(f, "click", function () {
				...
				e) {
				if (!e._basicStyle)
					e._basicStyle = e.element.getAttribute("style");
			...
			e && e._basicStyle && e.element.setAttribute("style", e._basicStyle);
		return f
	});
	...
	for (r in n)
		n[r].prototype.supportsDrilldown && q(n[r].prototype, "drawTracker", o)
})(Highcharts);
