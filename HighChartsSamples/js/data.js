...
(function (j) {
	...
	j.extend(n.prototype, {
		...
		parseCSV : function () {
			...
			i = a.startColumn || 0,
			h = a.endColumn || Number.MAX_VALUE,
			g,
			k,
			o = 0;
		...
		parseTable : function () {
			...
			f = b.startColumn || 0,
			i = b.endColumn || Number.MAX_VALUE;
			a && (typeof a === "string" && (a = document.getElementById(a)), m(a.getElementsByTagName("tr"), function (a,
						b) {
					b >= d && b <= e && m(a.children, function (a, e) {
						if ((a.tagName === "TD" || a.tagName === "TH") && e >= f && e <= i)
							c[e - f] || (c[e - f] = []), c[e - f][b - d] = a.innerHTML
					})
				}), this.dataFound())
		},
		parseGoogleSpreadsheet : function () {
			...
			c && jQuery.ajax({
				dataType : "json",
				url : "https://spreadsheets.google.com/feeds/cells/" + c + "/" + (a.googleSpreadsheetWorksheet || "od6") + "/public/values?alt=json-in-script&callback=?",
				error : a.error,
				success : function (a) {
					var a = a.feed.entry,
					c,
					...
					for (l = 0; l < m; l++)
						if (l >= i && l <= h)
							d[l - i] = [], d[l - i].length = Math.min(n, f - e);
					for (l = 0; l < j; l++)
						if (c = a[l], g = c.gs$cell.row - 1, k = c.gs$cell.col - 1, k >= i && k <= h && g >= e && g <= f)
							d[k - i][g - e] = c.content.$t;
					b.dataFound()
				}
			})
		},
		...
		parseTypes : function () {
			for (var b =
					this.columns, a = b.length, c, d, e, f; a--; )
				for (c = b[a].length; c--; )
					d = b[a][c], e = parseFloat(d), f = this.trim(d), f == e ? (b[a][c] = e, e > 31536E6 ? b[a].isDatetime = !0 : b[a].isNumeric = !0) : (d = this.parseDate(d), a === 0 && typeof d === "number" && !isNaN(d) ? (b[a][c] = d, b[a].isDatetime = !0) : b[a][c] = f === "" ? null : f)
		},
		dateFormats : {
			"YYYY-mm-dd" : {
		...
		complete : function () {
			...
			if (d.complete || d.afterComplete) {
				...
				for (h = 0, k = 0; h < b.length; k++) {
					...
					i = [];
					if (h + e <= b.length)
						for (g = 0; g < b[h].length; g++)
							i[g] = [a[g], b[h][g] !== void 0 ? b[h][g] : null], e > 1 && i[g].push(b[h + 1][g] !== void 0 ? b[h + 1][g] : null), e > 2 && i[g].push(b[h + 2][g] !== void 0 ? b[h + 2][g] : null), e > 3 && i[g].push(b[h + 3][g] !== void 0 ? b[h + 3][g] : null), e > 4 && i[g].push(b[h + 4][g] !== void 0 ? b[h + 4][g] : null);
					f[k] = {
						name : b[h].name,
						data : i
					};
		...
		a && a.data ? j.data(j.extend(a.data, {
				afterComplete : function (e) {
					...
					if (a.hasOwnProperty("series"))
						if (typeof a.series === "object")
							for (f = Math.max(a.series.length, e.series.length); f--; )
								i = a.series[f] || {},
					a.series[f] = j.merge(i, e.series[f]);
					else
						delete a.series;
					a = j.merge(e, a);
					b.call(d, a, c)
