...
(function () {
		...
		for (var c = "{", d = !1, e, f, g, h, i, j = []; (c = a.indexOf(c)) !== -1; ) {
			...
			if (d) {
				...
				for (h = 0; h < i; h++)
					e = e[g[h]];
				if (f.length)
					f = f.join(":"), g = /\.([0-9])/, h = E.lang, i = void 0, /f$/.test(f) ? (i = (i = f.match(g)) ? i[1] : -1, e !== null && (e = Ga(e, i, h.decimalPoint, f.indexOf(",") > -1 ? h.thousandsSep : ""))) : e = cb(f, e)
			}
			j.push(e);
			a = a.slice(c + 1);
			c = (d = !d) ? "}" : "{"
	...
	Gb,
	F = {},
	R = I.Highcharts = I.Highcharts ? ra(16,
			!0) : {};
	cb = function (a, b, c) {
		if (!r(b) || isNaN(b))
			return "Invalid date";
	...
	P.prototype = {
		...
		show : function (a) {
			...
			xa ? (this.element.removeAttribute("visibility"), this) : this.attr({
	...
	ta.prototype = {
					...
					p(g, function (e) {
						if (e !== "" || g.length === 1) {
							...
							if (e !== " ") {
								...
								n++;
								if (l)
									for (var e = e.replace(/([^\^])-/g, "$1- ").split(" "), o = e.length > 1 && k.whiteSpace !== "nowrap", $, r, B = a._clipHeight, q = [], v = s(), t =
											1; o && (e.length || q.length); )
		...
		button : function (a, b, c, d, e, f, g, h, i) {
			var j = this.label(a,
					b, c, i, null, null, null, null, "button"),
			k = 0,
			l,
			o,
			n,
			...
			if (!d)
				a.xSetter = function (a, b, c) {
					...
					for (f = 1; f < d.length; f++)
						e = d[f], e.getAttribute("x") === c.getAttribute("x") &&
						e.setAttribute("x", a);
					c.setAttribute(b, a)
				};
			return a
		...
		label : function (a, b, c, d, e, f, g, h, i) {
			function j() {
				...
				if (z) {
					if (!m)
						a = u(-L * x), b = h ? -na : 0, n.box = m = d ? o.symbol(d,
	...
	Sa.prototype = {
		...
		render : function (a, b, c) {
			...
			if (i && !isNaN(y))
				i.xy = w = this.getLabelPosition(y, w, i, g, k, u, a, v), this.isFirst && !this.isLast &&
	...
	la.prototype = {
		...
		setOptions : function (a) {
			this.options = w(this.defaultOptions, this.isXAxis ? {}
					 ...
					 : this.defaultYAxisOptions, [this.defaultTopAxisOptions, this.defaultRightAxisOptions, this.defaultBottomAxisOptions, this.defaultLeftAxisOptions][this.side], w(E[this.coll], a))
		},
		...
		setTickPositions : function (a) {
			...
			if (b.range && r(b.max))
				b.userMin = b.min = v(b.min, b.max - b.range), b.userMax = b.max, b.range =
					null;
			...
			if (!a)
				!b.ordinalPositions && (b.max - b.min) / b.tickInterval > v(2 * b.len, 200) && ra(19, !0), a = f ? b.getTimeTicks(b.normalizeTimeTickInterval(b.tickInterval, d.units), b.min, b.max, d.startOfWeek, b.ordinalPositions, b.closestPointRange,
		...
		setMaxTicks : function () {
			...
			if (!this.isLinked &&
				!this.isDatetimeAxis && c && c.length > (b[d] || 0) && this.options.alignTicks !== !1)
				b[d] = c.length;
			a.maxTicks = b
		},
		...
		getTitlePosition : function () {
			...
			[e.align],
			b = (a ? c + this.height : b) + (a ? 1 : -1) * (g ? -1 : 1) *
				...
				if (a.minorTickInterval && !a.categories && p(a.getMinorTickPositions(), function (b) {
						...
						o[b].render(null, !1, 1)
							...
							if (!h || b >= a.min && b <= a.max)
								l[b] || (l[b] = new Sa(a, b)) , x && l[b].isNew && l[b].render(c,
	...
	la.prototype.getTimeTicks = function (a, b,
		...
		if (r(b)) {
			...
			h = i[gb]();
			for (var d =
					i.getTime(), l = i[fb](), o = i[Xa](), n = g ? Ra : (864E5 + i.getTimezoneOffset() * 6E4) % 864E5; d < c; )
	...
	la.prototype.normalizeTimeTickInterval = function (a, b) {
		var c = b || [["millisecond", [1, 2, 5, 10, 20, 25, 50, 100, 200, 500]], ["second", [1, 2, 5, 10, 15, 30]], ["minute",
					[1, 2, 5, 10, 15, 30]], ["hour", [1, 2, 3, 4, 6, 8, 12]], ["day", [1, 2]], ["week", [1, 2]], ["month", [1, 2, 3, 4, 6]], ["year", null]],
		d = c[c.length - 1],
		e = A[d[0]],
		f = d[1],
		g;
	...
	la.prototype.getLogTickPositions = function (a, b, c, d) {
		...
		if (a >= 0.5)
			a = u(a), g = this.getLinearTickPositions(a,
					b, c);
		else if (a >= 0.08)
	...
	Wa.prototype = {
			...
			if (d && this.options.tooltip.shared && (!h || !h.noSharedTooltip)) {
				...
				for (j = 0; j < i; j++)
					if (c[j].visible && c[j].options.enableMouseTracking !== !1 && !c[j].noSharedTooltip &&
						c[j].singularTooltips !== !0 && c[j].tooltipPoints.length && (e = c[j].tooltipPoints[l]) && e.series)
		...
		drop : function (a) {
			...
			if (b)
				G(b.container, {
					cursor : b._cursor
				}), b.cancelClick = this.hasDragged > 10, b.mouseIsDown = this.hasDragged = this.hasPinched =
					!1, this.pinchDown = []
		},
		...
		onContainerMouseLeave : function () {
			var a = V[oa];
			if (a)
				a.pointer.reset(), a.pointer.chartPosition =
					null
		},
	...
	if (I.PointerEvent || I.MSPointerEvent) {
		...
		Ab = function (a, b, c, d) {
			...
			if ((a.pointerType === "touch" ||
					a.pointerType === a.MSPOINTER_TYPE_TOUCH) && V[oa])
	...
	lb.prototype = {
		init : function (a, b) {
			...
			this.options = b;
			if (b.enabled)
				c.baseline = z(d.fontSize) + 3 + f, c.itemStyle = d, c.itemHiddenStyle = w(d, b.itemHiddenStyle), c.itemMarginTop = f, c.padding = e, c.initialItemX = e, c.initialItemY =
		...
		processData : function (a) {
			...
			if (l && this.sorted && (!j || d > j || this.forceCrop))
				if (o = h.min, n = h.max, b[d - 1] < o || b[0] > n)
					...
					e = this.cropData(this.xData, this.yData, o, n), b = e.xData, c = e.yData,
		...
		translate : function () {
			...
			for (var a = this.options, b = a.stacking, c = this.xAxis, d = c.categories, e = this.yAxis, f = this.points, g = f.length, h = !!this.modifyValue, i = a.pointPlacement, j = i === "between" || ha(i), k = a.threshold, a = 0; a < g; a++) {
				...
				if (b && this.visible && p && p[o])
					p = p[o], n = p.points[this.index + "," + a], s = n[0], n = n[1], s === 0 && (s = m(k, e.min)),
			...
			if (!i || g < i || k)
				for (; g--; ) {
					...
					if (k) {
						...
						f = d.hover = d.hover || {};
						if (!b.marker)
							f.color = f.color || !i.options.color && e.color ||
	...
	O.prototype.setStackedPoints = function () {
		if (this.options.stacking && !(this.visible !== !0 && this.chart.options.chart.ignoreHiddenSeries !== !1)) {
			...
			for (q = 0; q < d; q++) {
				...
				l[m] || (l[m] = {});
				if (!l[m][r])
					o[m] && o[m][r] ? (l[m][r] = o[m][r], l[m][r].total = null) : l[m][r] = new Hb(k,
	...
	O.prototype.setPercentStacks = function () {
		...
		p([b, "-" + b], function (b) {
			...
			for (var f = d.length, g, h; f--; )
				if (g =
						d[f], e = (h = c[b] && c[b][g]) && h.points[a.index + "," + f], g = e)
	...
	O.prototype.drawDataLabels = function () {
		...
		if (d.enabled || a._hasPointLabels)
			a.dlProcessOptions && a.dlProcessOptions(d), i = a.plotGroup("dataLabelsGroup",
					"data-labels", "hidden", d.zIndex || 6), !a.hasRendered && m(d.defer, !0) && (i.attr({
					opacity : 0
				...
				for (D = 2; D--; ) {
					...
					for (B = 0; B < I; B++) {
						...
						if (this.options.size === null)
							w = t.width, A - w < f ? F[3] = v(u(w - A + f), F[3]) : A + w > h - f && (F[1] = v(u(A + w - h + f), F[1])), C - y / 2 < 0 ? F[0] =
				...
				for (i = 0; i < a; i++)
					if (e = b[i], c = e.x, c >= f.min && c <= f.max) {
						...
						for (d = b[i + 1] ? C(v(0, T((e.clientX +
												(h ? h.wrappedClientX || h.clientX : g)) / 2)), g) : g; c >= 0 && c <= d; )
							j[c++] = e
					}
